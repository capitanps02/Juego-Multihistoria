...
export function mountGame({root, GameSession, assets, css, storageKey='historia-jugador.preview.session.v1', events}) {
  ...
  let session=null, busy=false, view='home', cinematic=false, message='', replacement=null, lastArt='stadium_bg';
  let savedRaw=null, autoTimer=null;
  ...
  function simulationMode(){return session?.getView?.().simulation?.mode||'idle';}
  function simulationPaused(){return simulationMode()==='paused';}
  function interruptionText(type){return ({
    decision:'Hay una decisión que necesita tu respuesta.',
    offer:'Ha llegado una oferta que necesita tu respuesta.',
    ...
    season_transition:'Ha cambiado la temporada o una etapa de tu carrera.',
    retirement:'Tu carrera ha alcanzado su cierre.',
    max_auto_weeks:'El tramo automático ha llegado a su límite.'
  })[type]||'La simulación se ha detenido en un momento relevante.';}
  function button(text,fn,cls='secondary',options={}){const b=el('button',undefined,cls);b.type='button';b.append(el('span',text));b.disabled=busy||(simulationPaused()&&options.blockedWhenPaused===true);if(options.ariaLabel)b.setAttribute('aria-label',options.ariaLabel);if(options.pressed!==undefined)b.setAttribute('aria-pressed',String(options.pressed));b.addEventListener('click',fn);return b;}
  ...
  function pauseToggle(){if(!session||busy)return;const mode=simulationMode();if(mode==='auto_simulating')run('auto',{action:'pause'});else if(mode==='paused')run('auto',{action:'resume'});}
  ...
  function queueAutoStep(){
    ...
    if(!session||simulationMode()!=='auto_simulating')return;
    autoTimer=setTimeout(()=>{
      if(!busy&&session&&simulationMode()==='auto_simulating')run('auto',{action:'step'});
    },120);
  }
  ...
  function mainAction(v){
    ...
    if(v.screen==='summary')return button('Continuar simulando',()=>run('auto',{action:'start'}),'primary');
    if(v.simulation.mode==='auto_simulating')return button('Pausar',()=>run('auto',{action:'pause'}),'primary');
    if(v.simulation.mode==='paused')return button('Reanudar',()=>run('auto',{action:'resume'}),'primary');
    return button('Simular',()=>run('auto',{action:'start'}),'primary');
  }
  ...
  function home(v,main){const grid=el('div',undefined,'home-grid');grid.append(hero(v));const next=panel(v.screen==='epilogue'?'El final de un capítulo':v.screen==='summary'?'Resumen del periodo':'Lo que viene ahora');next.classList.add('next');next.append(photo('stadium_bg','card-bg'));const content=el('div',undefined,'next-content');const period=v.simulation.summary;content.append(el('span',date(v.date),'eyebrow'),el('h2',v.offer?.reason||v.decision?.title||v.result?.title||(v.screen==='epilogue'?'Una carrera para recordar':v.screen==='summary'&&period?`${period.daysSimulated} días después`:'El siguiente paso.')),
      el('p',v.offer?'Hay una propuesta de contrato que necesita tu respuesta.':v.decision?'Hay un momento que necesita tu respuesta.':v.result?'Tu decisión ya forma parte de esta historia.':v.screen==='epilogue'?'Mira atrás y recorre los momentos que te han traído hasta aquí.':v.screen==='summary'&&period?`Apariciones +${period.matches.appearances} · Forma ${period.playerChanges.form>=0?'+':''}${period.playerChanges.form} · Fatiga ${period.playerChanges.fatigue>=0?'+':''}${period.playerChanges.fatigue} · Estado físico ${period.playerChanges.fitness>=0?'+':''}${period.playerChanges.fitness}.`:'Entrenamientos, partidos y conversaciones. Simula hasta el próximo momento que necesite tu atención.','muted'),mainAction(v));next.append(content);grid.append(next,stats(v));if(v.screen==='summary'&&period){const summary=panel('Qué ha cambiado');for(const text of period.worldHighlights.slice(-4))summary.append(el('p',text,'muted'));if(period.interruption)summary.append(el('p',interruptionText(period.interruption.type),'muted'));grid.append(summary);}
    ...
    const tutorial=panel('Cómo se juega');tutorial.classList.add('tutorial');tutorial.append(el('p','Pulsa Simular y el tiempo avanzará automáticamente. Se detendrá ante decisiones, ofertas, lesiones importantes, transiciones o al completar el bloque; puedes pausarlo entre semanas.','muted'));grid.append(tutorial);
  ...
  function render(focus=false){const v=session?.getView();shell.replaceChildren();shell.classList.toggle('immersive',cinematic&&!!v);const header=el('header',undefined,'topbar');const brand=el('div',undefined,'brand');brand.append(el('span','M','brand-mark'));const name=el('div');name.append(el('strong','Multihistoria'),el('small','Carrera de futbolista'));brand.append(name);const mode=v?.simulation?.mode;const pause=v&&['auto_simulating','paused'].includes(mode)?button(mode==='paused'?'Reanudar':'Pausar',pauseToggle,'secondary',{ariaLabel:mode==='paused'?'Reanudar la simulación':'Pausar la simulación',pressed:mode==='paused'}):null;if(pause)pause.classList.add('pause-button');header.append(brand,el('span',v?'Temporada '+v.season.replace('-',' / 20'):'Una vida. Mil decisiones.','chapter'),...(pause?[pause]:[]),button(v?date(v.date):'Tu partida',()=>navigate('save'),'date-button'));shell.append(header);
