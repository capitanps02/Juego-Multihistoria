...
export function assertSessionSnapshot(value, events) {
    validateData(value);
    ...
    if (state.market?.pending)
        ensure(!s.pendingDecision && !s.pendingResult && !s.needsWorldAdvance && state.retirement.status !== "closed", "market.pending", "oferta incompatible con pantalla");
