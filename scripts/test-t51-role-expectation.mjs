...
test('role expectation/3 solo la postura role_guarantees del bridge prueba expectativa concreta de rol', () => {
  const state = age23State(9803);
  resolveBridge(state, 'A');
  assert.equal(hasRoleGuaranteeAt23(state), true);

  const other = age23State(9804);
  resolveBridge(other, 'B');
  assert.equal(hasRoleGuaranteeAt23(other), false, 'security_over_role no debe convertirse en promesa de minutos');
});
...
test('role expectation/6 narrativeCausalFacts separa evidencia de promesa y magnitud de caída', () => {
