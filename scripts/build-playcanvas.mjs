...
const ui=fs.readFileSync(path.join(root,'web/game-ui.js'),'utf8').replace("import { createIndexedSaveStore } from './indexed-save-store.js';",'').replace("import { formatClubName } from './club-names.js';",'').replace('export function mountGame','function mountGame');
const clubCatalogNames=fs.readFileSync(path.join(root,'web/club-catalog-names.js'),'utf8').replace('export const CATALOG_CLUB_NAMES','const CATALOG_CLUB_NAMES');
const clubNames=fs.readFileSync(path.join(root,'web/club-names.js'),'utf8').replace("import { CATALOG_CLUB_NAMES } from './club-catalog-names.js';",'').replace('export function formatClubName','function formatClubName');
...
const manifest={integration:'T3.2',targetScene:2593315,moduleCount:modules.size,bundleBytes:Buffer.byteLength(bundle),bundleSha256:createHash('sha256').update(bundle).digest('hex'),inputs:Object.fromEntries([...modules.keys(),'web/game-ui.js','web/club-catalog-names.js','web/club-names.js','web/game-ui.css','web/indexed-save-store.js'].map(p=>[p,createHash('sha256').update(fs.readFileSync(path.join(root,p))).digest('hex')]))};
fs.writeFileSync(path.join(root,'playcanvas/manifest.json'),JSON.stringify(manifest,null,2)+'\n');console.log(JSON.stringify({modules:modules.size,bytes:manifest.bundleBytes,sha256:manifest.bundleSha256}));
