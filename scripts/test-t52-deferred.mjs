...
test('origin-club proof: teammate-cover precedent cannot leak into a new club', async () => {
  const { EVENTS } = await import('../dist/content/events/index.js');
  const { createInitialState } = await import('../dist/content/initial-state.js');
  const { expireDueSeedsInPlace } = await import('../dist/narrative/resolver.js');
  const { eventGatesPass } = await import('../dist/narrative/event-gates.js');
  ...
  const producer = EVENTS.find(event => event.id === proof.producerEventId);
  const consumer = EVENTS.find(event => event.id === proof.consumerEventId);
  assert.ok(producer, 'falta el productor canónico del precedente de cobertura');
  assert.ok(consumer, 'falta el consumidor canónico del precedente de cobertura');
  assert.ok(
    producer.outcomes.some(outcome => (outcome.seedTransitions ?? []).some(transition => transition.seedId === proof.seedId && transition.action === 'create')),
    'el productor acreditado debe crear SEED_TEAMMATE_COVER'
  );
  ...
  state.seeds.push({
    ...
    originSeason: state.season,
    npcRefs: [],
    payload: { __t52OriginClub: state.club, stance: 'refuse_without_exposing' },
    lastTouchedDate: state.date
  });
  state.flags.HAS_SEED_TEAMMATE_COVER = true;

  assert.equal(eventGatesPass(state, consumer), true, 'la memoria local puede abrir la decisión mientras continúa en el club de origen');
  ...
  assert.equal(eventGatesPass(state, consumer), false, 'el precedente de otro vestuario no puede habilitar la escena en el nuevo club');
});
