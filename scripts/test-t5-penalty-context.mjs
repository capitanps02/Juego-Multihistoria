...
function stateWithPenaltySetup() {
  for (let seed = 9100; seed < 12000; seed += 1) {
    ...
    if (setup) return { state, match, setup, seed };
  }
  throw new Error('No deterministic penalty setup found in directed seed range');
}

test('penalty setup/1 first materialization and replay are deterministic and consume zero RNG draws', () => {
  const { state, match, setup, seed } = stateWithPenaltySetup();

  const fresh = appearedMatchState(seed);
  const beforeFirst = structuredClone(fresh.state.rngState);
  const first = recordPenaltyDecisionSetupInPlace(fresh.state, fresh.match);
  assert.deepEqual(first, setup);
  assert.deepEqual(fresh.state.rngState, beforeFirst, 'first setup materialization must consume zero RNG draws');

  const beforeReplay = structuredClone(state.rngState);
  ...
  assert.deepEqual(state.rngState, beforeReplay, 'idempotent replay must consume zero RNG draws');
});
...
test('penalty setup/8 decision can never occur after a starting player has left the field', () => {
  let checkedStarters = 0;
  for (let seed = 9100; seed < 12000; seed += 1) {
    const { state, match } = appearedMatchState(seed);
    if (!match.player.started || match.player.minutes >= 90) continue;
    checkedStarters += 1;
    const setup = recordPenaltyDecisionSetupInPlace(state, match);
    if (setup) assert.ok(setup.decisionMinute <= match.player.minutes);
    if (checkedStarters >= 50) break;
  }
  assert.ok(checkedStarters > 0, 'directed range should include substituted starters');
});


test('penalty setup/9 same-id caller object cannot fabricate appearance against authoritative match row', () => {
  const { seed } = stateWithPenaltySetup();
  const state = matchDayState(seed);
  const authoritative = recordOfficialMatchInPlace(state, {
    appeared: false,
    debutOccurred: false,
    injuryUnavailable: false
  });
  assert.ok(authoritative);
  assert.equal(authoritative.player.appeared, false);

  const forged = structuredClone(authoritative);
  forged.player.calledUp = true;
  forged.player.onBench = true;
  forged.player.started = false;
  forged.player.appeared = true;
  forged.player.minutes = 40;

  const before = structuredClone(state);
  assert.equal(recordPenaltyDecisionSetupInPlace(state, forged), null);
  assert.equal(state.world.sportPenaltySetups, undefined);
  assert.deepEqual(state, before, 'forged caller data must not mutate sporting authority');
});
