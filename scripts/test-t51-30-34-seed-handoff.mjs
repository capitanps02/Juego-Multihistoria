...
test('the remaining two orphan seeds stay unwired until their canonical scenes exist', () => {
...
test('pending orphan seed ownership remains one-to-one', () => {
  ...
  assert.equal(Object.keys(PENDING_CANONICAL_WRITERS).length, 2);
});


test('SEED_SPECIALIST_BIGCLUB now has exactly its canonical EVT_30_JAN_001 writer', () => {
  assert.deepEqual(seedWriters('SEED_SPECIALIST_BIGCLUB'), ['EVT_30_JAN_001']);
  assert.equal(EVENTS.some(event => event.id === 'EVT_30_JAN_001'), true);
});
