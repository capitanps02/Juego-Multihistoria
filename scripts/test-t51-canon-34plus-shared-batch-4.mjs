...
function stateAt(){const s=createInitialState(21001);s.age=40;s.phase='34_plus';s.retirement.status='playing';return s;}
function addOffer(s){const before=careerTerms(s);const terms={...before,salary:before.salary+1,months:Math.max(12,before.months)};s.market={version:1,sequence:1,pending:{id:'offer:dual',date:s.date,reason:'Dual role',before,terms},history:[]};}
...
test('shared batch 4 fails closed without owner facts',()=>{const s=stateAt();for(const id of expected)assert.equal(isStagedSharedBatch4Eligible(s,id,false),false);});
test('dual-role scene also requires a real eligible formal offer',()=>{const s=stateAt();assert.equal(isStagedSharedBatch4Eligible(s,'EVT_35_DUAL_001',true),false);addOffer(s);assert.equal(isStagedSharedBatch4Eligible(s,'EVT_35_DUAL_001',true),true);});
test('remaining shared scenes may open only after their exact external fact adapter',()=>{const s=stateAt();for(const id of expected.filter(x=>x!=='EVT_35_DUAL_001'))assert.equal(isStagedSharedBatch4Eligible(s,id,true),true,id);});
