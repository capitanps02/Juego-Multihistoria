...
test('shared batch 3 fails closed without owner facts',()=>{const s=createInitialState(20001);s.age=40;s.phase='34_plus';s.retirement.status='playing';for(const id of expected){assert.equal(isStagedSharedBatch3Eligible(s,id,false),false);assert.equal(isStagedSharedBatch3Eligible(s,id,true),true);}});
