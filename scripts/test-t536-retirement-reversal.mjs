...
function putOffer(state,id,date,salaryDelta=100){
  ...
  const offer=proposePostAnnouncementCareerChange(
    ...
    draft=>{
      ...
      draft.contract.salaryMonthly=Math.max(1000,Number(draft.contract.salaryMonthly)+salaryDelta);
    }
  );
  assert.ok(offer,'formal post-announcement CareerOffer must materialize');
