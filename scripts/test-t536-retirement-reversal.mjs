...
test('T5.36 Stage A requires a real post-announcement CareerOffer and Stage B is exclusive',()=>{
  ...
  putOffer(state,'offer:post-a','2046-03-15');
  ...
  respondToOffer(state,'offer:post-a','defer',{kind:'narrative_choice',historyIndex:state.history.length-1,eventId:stageA.id,choiceId:'ACKNOWLEDGE'});
  putOffer(state,'offer:post-b','2046-04-01',200);
...
test('T5.36 canonical reversal is the only announced->playing path and charges costs once',()=>{
  ...
  resolveChoiceInPlace(state,stageA,'ACKNOWLEDGE');
  respondToOffer(state,'offer:post-a','defer',{kind:'narrative_choice',historyIndex:state.history.length-1,eventId:stageA.id,choiceId:'ACKNOWLEDGE'});
  putOffer(state,'offer:post-b','2046-04-01',300);
  ...
  respondToOffer(state,'offer:post-b','accept',{kind:'narrative_choice',historyIndex:state.history.length-1,eventId:reversal.id,choiceId:'ACCEPT'});
...
test('T5.36 reversal window survives save/load and closed careers cannot reopen',()=>{
  ...
  resolveChoiceInPlace(state,stageA,'ACKNOWLEDGE');
  respondToOffer(state,'offer:post-a','defer',{kind:'narrative_choice',historyIndex:state.history.length-1,eventId:stageA.id,choiceId:'ACKNOWLEDGE'});
  putOffer(state,'offer:post-b','2046-04-01',250);

  const restored=loadSave(serializeSave(state));
  assert.equal(retirementPostAnnouncementOfferFact(restored).stage,'reversal');
  assert.equal(restored.market.pending.id,'offer:post-b');
