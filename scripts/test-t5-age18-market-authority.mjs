...
test('UDV relegation keeps live league tier coherent before a formal summer offer snapshot', () => {
  let relegated = null;
  for (let seed = 1; seed <= 4096 && !relegated; seed += 1) {
    const state = createInitialState(seed);
    state.date = '2027-05-01';
    state.runtime.day = 6;
    state.runtime.seasonDay = 304;
    state.world.clubPressure = 100;
    state.sport.form = 0;
    advanceWorldDayInPlace(state);
    if (state.flags.UDV_RELEGATED === true) relegated = state;
  }

  assert.ok(relegated, 'fixture must find a deterministic UDV relegation case');
  assert.equal(relegated.club, 'UDV');
  assert.equal(relegated.tier, 4);
  assert.equal(relegated.professional.leagueTier, 4);
  assert.equal(careerTerms(relegated).tier, careerTerms(relegated).leagueTier);

  relegated.date = `2027-${AGE18_SUMMER_OFFER_MONTH_DAY}`;
  relegated.market.pending = null;
  relegated.reputation.marketHeat = 10;
  relegated.sport.appearances = 2;
  relegated.flags.OFFICIAL_DEBUT = true;
  assert.equal(materializeAge18MarketOfferInPlace(relegated), 'renewal');
  assert.equal(relegated.market.pending.before.tier, 4);
  assert.equal(relegated.market.pending.before.leagueTier, 4);
});
