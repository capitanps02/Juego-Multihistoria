...
test('UDV financial crisis needs factual home institution memory plus UDV/home relevance',()=>{const s=createInitialState(17001);s.age=35;s.phase='34_plus';s.retirement.status='playing';s.club='Away FC';s.professional.registrationClub='Away FC';s.professional.ownerClub='Away FC';s.professional.route='domestic';assert.equal(isUdvFinancialCrisisEligible(s),false);s.seeds.push(seed());assert.equal(isUdvFinancialCrisisEligible(s),false);s.professional.route='home';assert.equal(isUdvFinancialCrisisEligible(s),true);});
