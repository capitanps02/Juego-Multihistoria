...
export class DeterministicRng {
  constructor(private stream: RngStreamState) {}
  ...
  pickWeighted<T>(items: Array<{ item: T; weight: number }>): { item: T; draw: number } {
    const valid = items.filter(x => Number.isFinite(x.weight) && x.weight > 0);
    if (!valid.length) throw new Error("No weighted candidates available");
    const total = valid.reduce((s, x) => s + x.weight, 0);
    const draw = this.next();
    let cursor = draw * total;
    for (const x of valid) {
      cursor -= x.weight;
      if (cursor <= 0) return { item: x.item, draw };
    }
    return { item: valid[valid.length - 1]!.item, draw };
  }
}

export function makeRngStream(seed: number, salt: number): RngStreamState {
