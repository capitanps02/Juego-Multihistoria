...
const errors=validateBuild().filter(x=>x.level==="error");if(errors.length)throw new Error(`Build validation failed:\n${JSON.stringify(errors,null,2)}`);
const a=simulateCareer({seed:424242,days:2922});const b=simulateCareer({seed:424242,days:2922});if(a.signature!==b.signature)throw new Error("Reproducibility failure: same seed generated different careers/final state.");
