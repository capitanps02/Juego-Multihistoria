...
const o=(id:string,effects:any[]=[],messages:string[]=[]):OutcomeDefinition=>({id:`${id}_OUT`,baseWeight:1,effects,messages:messages.length?messages:["El contexto tardío cambia el siguiente tramo de la carrera."]});
...
function generic(id:string,minAge:number,title:string,gates:any[]=[],months?:number[],weight=.9):EventDefinition{
...
postOffer.outcomes=postOffer.choices.map(x=>o(x.id)); postOffer.weight=7; postOffer.tags=[...(postOffer.tags??[]),"retirement_terminal"];
