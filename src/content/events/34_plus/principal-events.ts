...
const o=(id:string,effects:any[]=[],messages:string[]=[]):OutcomeDefinition=>({id:`${id}_OUT`,baseWeight:1,effects,messages:messages.length?messages:["La decisión cambia el equilibrio de tu tramo final."]});
...
function generic(id:string,age:number,title:string,family:EventDefinition["family"],months:number[],weight=1):EventDefinition{
  const choices=[
...
const principal:EventDefinition[]=[
...
for(const id of ["EVT_34_TEAM_001","EVT_34_HOME_001","EVT_34_MED_001","EVT_34_LEG_001","EVT_34_PRS_001","EVT_35_HOME_001","EVT_35_FINAL_001","EVT_35_PRS_001","EVT_36_RICH_001"]){ const ev=principal.find(x=>x.id===id); if(ev) ev.weight*=1.35; }
...
veteranMarket.repeatable=true; veteranMarket.cooldown=180; veteranMarket.weight=2.2;
...
function retirementEvent(id:string,title:string,gates:any[],choices:ChoiceDefinition[],outcomes:OutcomeDefinition[],months?:number[],repeatable=false,weight=2):EventDefinition{
...
principal.push(
  retirementEvent("EVT_RET_HOME_001","La conversación en casa",[{path:"retirement.status",op:"eq",value:"playing"},{path:"professional.retirementDistance",op:"gte",value:30}],
    [c("KEEP","Quiero seguir",[n("professional.motivationReserve",4)]),c("DECIDE","Creo que ha llegado",[s("retirement.status","decided"),s("retirement.decidedDate",null),s("retirement.reason","voluntary")])],
    [o("KEEP"),o("DECIDE",[f("RETIREMENT_DECISION_CONTEXT")])],undefined,false,1.4),
  retirementEvent("EVT_RET_BODY_001","El cuerpo dice basta, quizá",[{path:"retirement.status",op:"eq",value:"playing"},{path:"flags.LATE_BODY_REDLINE",op:"eq",value:true}],
    [c("HEALTH","Parar por salud",[s("retirement.status","decided"),s("retirement.reason","health"),f("HEALTH_RETIREMENT_CONTEXT")]),c("ONE_MORE","Intentar una temporada más",[n("professional.motivationReserve",-4),n("professional.matchSelectivity",8)])],
    [o("HEALTH"),o("ONE_MORE")],undefined,false,1.8),
  retirementEvent("EVT_RET_HIGH_001","Retirarte después de ganar",[{path:"retirement.status",op:"eq",value:"playing"},{path:"flags.RETIRE_AFTER_WIN_CONTEXT",op:"eq",value:true}],
    [c("HIGH","Irme arriba",[s("retirement.status","decided"),s("retirement.reason","retire_on_high"),f("RETIRE_ON_HIGH")]),c("CONTINUE","Seguir",[n("professional.motivationReserve",3)])],
    [o("HIGH"),o("CONTINUE")],undefined,false,1.5),
  retirementEvent("EVT_RET_LOW_001","Retirarte después de caer",[{path:"retirement.status",op:"eq",value:"playing"},{path:"flags.RETIRE_AFTER_LOW_CONTEXT",op:"eq",value:true}],
    [c("LOW","Aceptar el final",[s("retirement.status","decided"),s("retirement.reason","retire_on_low"),f("RETIRE_ON_LOW")]),c("FIGHT","Buscar otro sitio",[n("professional.motivationReserve",2),n("reputation.marketHeat",2)])],
    [o("LOW"),o("FIGHT")],undefined,false,1.6),
  retirementEvent("EVT_RET_ANNOUNCE_001","Quién se entera primero",[{path:"retirement.status",op:"eq",value:"decided"}],
    [c("PUBLIC","Anunciarlo",[s("retirement.status","announced"),f("RETIREMENT_ANNOUNCED")]),c("PRIVATE","Decírselo primero a los tuyos",[s("retirement.status","announced"),f("RETIREMENT_ANNOUNCED")]),c("WAIT","Esperar un poco más",[])],
    [o("PUBLIC",[f("RETIREMENT_PUBLIC")]),o("PRIVATE",[f("RETIREMENT_PRIVATE")]),o("WAIT")],undefined,true,3.2),
  retirementEvent("EVT_RET_LAST_001","El último partido no está garantizado",[{path:"retirement.status",op:"eq",value:"announced"},{path:"flags.LAST_MATCH_WINDOW",op:"eq",value:true}],
    [c("PLAY","Jugar si el cuerpo y el entrenador lo permiten",[f("LAST_MATCH_PLAYED"),s("retirement.status","closed"),s("retirement.closureType","planned_last_match")]),c("NO_MATCH","Aceptar que quizá no haya despedida",[s("retirement.status","closed"),s("retirement.closureType","no_last_match")])],
    [o("PLAY"),o("NO_MATCH")],[4,5,6],false,6)
);
