...
const debutDecisionContext: Condition = {
  path: "facts.match.debutDecisionContext",
  op: "eq",
  value: true
};

const firstRealMatchSquadCall: Condition = {
  path: "facts.sport.firstMatchSquadCall",
  op: "neq",
  value: null
};

const futureTrainingExists: Condition = {
  path: "facts.sport.nextTrainingDate",
  op: "neq",
  value: null
};

const atLeastOneLeagueMatchRemaining: Condition = {
  path: "facts.sport.remainingLeagueMatches",
  op: "gte",
  value: 1
};

const atMostFourLeagueMatchesRemaining: Condition = {
  path: "facts.sport.remainingLeagueMatches",
  op: "lte",
  value: 4
};

const leagueObjectiveOpen: Condition = {
 ...
 */
export function applyT51124SportContextRepairs(events: EventDefinition[]): EventDefinition[] {
  ...
  if (missing.length) throw new Error(`T5.1 #124 missing expected 18-20 event(s): ${missing.join(", ")}`);
  return repaired;
}
