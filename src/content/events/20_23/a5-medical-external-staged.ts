...
const EVT_20_MED_001 = ambiguousEvent({
  ...
  choices:[
    ...
    {id:"MINIMIZE",label:"Minimizar un episodio que consideras irrelevante",intentTags:["medical","minimize","risk"],primaryMessage:"Tratas una molestia no diagnosticada como un episodio menor, sin afirmar que no existió.",secondaryMessage:"Si aparece documentación más tarde, el problema puede pasar de médico a credibilidad.",primaryEffects:[n("control.career",1)],secondaryEffects:[n("professional.environmentStability",-3),n("professional.institutionalTrust",-2)],primarySeedTransitions:[seedCreate("SEED_MEDICAL_DISCLOSURE",64,{stance:"minimize"})],secondarySeedTransitions:[seedCreate("SEED_MEDICAL_DISCLOSURE",72,{stance:"minimize",credibility_risk:true})]},
...
const EVT_21_MED_001 = ambiguousEvent({
  ...
  weight:13,
  choices:[
    {id:"PLAY_TREATMENT",label:"Jugar con el tratamiento",intentTags:["medical","risk","match"],primaryMessage:"Aceptas el alivio sintomático sabiendo que no equivale a curación.",secondaryMessage:"La decisión te mantiene disponible, pero puede alargar la recuperación si el cuerpo responde peor de lo esperado.",primaryEffects:[n("control.career",2),n("professional.environmentStability",-1)],secondaryEffects:[n("professional.environmentStability",-4),n("control.career",-1)],primarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",4),intensify("SEED_BODY_PRECEDENT",4)],secondarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",6),intensify("SEED_BODY_PRECEDENT",8)]},
    {id:"UNAVAILABLE",label:"Declararte no disponible",intentTags:["medical","protect","availability"],primaryMessage:"Proteges recuperación y renuncias a una ventana deportiva sin fingir que el partido deja de importar.",secondaryMessage:"La decisión reduce riesgo físico, pero otra persona ocupa un espacio competitivo real.",primaryEffects:[n("professional.environmentStability",3),n("control.career",2)],secondaryEffects:[n("professional.environmentStability",1),n("reputation.marketHeat",-1)],primarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",3),intensify("SEED_BODY_PRECEDENT",2)],secondarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",4)]},
    {id:"SECOND_OPINION",label:"Pedir una segunda opinión rápida",intentTags:["medical","information"],primaryMessage:"Buscas información adicional antes de asumir el riesgo del tratamiento.",secondaryMessage:"La segunda opinión puede confirmar el plan y consumir parte del tiempo de decisión.",primaryEffects:[n("control.career",6),n("professional.environmentStability",1)],secondaryEffects:[n("control.career",3)],primarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",2)],secondarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",3)]},
    {id:"TRAIN_EVE",label:"Aceptar solo si entrenas sin empeorar en la víspera",intentTags:["medical","conditional","evidence"],primaryMessage:"Conviertes la disponibilidad en una condición observable en vez de una promesa.",secondaryMessage:"La prueba aporta evidencia y puede demostrar que todavía no estás listo.",primaryEffects:[n("control.career",5),n("professional.environmentStability",2)],secondaryEffects:[n("professional.environmentStability",-1),n("control.career",2)],primarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",3),intensify("SEED_BODY_PRECEDENT",2)],secondarySeedTransitions:[intensify("SEED_MEDICAL_DISCLOSURE",4)]}
  ],
