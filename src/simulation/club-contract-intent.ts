...
export function coachPromiseWaitFacts(state: GameState): CoachPromiseWaitFacts | null {
  ...
  if (!store) return { choiceDate: choice.date, club: choice.club, officialMatchesElapsed: 0, complete: false };
  const elapsed = store.fixtures.filter(row =>
    row.official === true && row.date > choice!.date && row.date <= state.date && row.club === choice!.club
  ).length;
