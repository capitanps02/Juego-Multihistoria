...
function adaptProfessionalContext(state: GameState, tags: State20Tag[]): void {
  if (state.professional.initializedAt20) return;

  const has = (tag: State20Tag) => tags.includes(tag);
  ...
  let route: GameState["professional"]["route"] = "domestic";
  ...
  if (state.flags.ABROAD_ROUTE) route = "abroad";
  if (state.flags.CONFLICT_EXIT) state.flags.CONTRACT_DISPUTE = true;
...
export function adaptState20ToProfessional(state: GameState, tags: State20Tag[]): void {
  if(state.professional.initializedAt20)return;
