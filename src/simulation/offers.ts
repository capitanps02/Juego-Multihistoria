...
export function isCareerOfferContext(value:unknown):value is CareerOfferContext{
  ...
  if(context.kind==="veteran_offer"){
    if(Object.keys(context).sort().join()!=="ancillaryRole,approachId,kind,opportunity,sportingRole")return false;
    ...
    return (context.approachId===null||boundedContextText(context.approachId))
      &&(context.sportingRole===null||boundedContextText(context.sportingRole))
