...
export interface OfferDecision {
  offer: CareerOffer; action: OfferAction; accepted: boolean; explanation: string;
}
...
export function respondToOffer(s: GameState, id: string, action: OfferAction): OfferDecision {
  const m=marketState(s),offer=m.pending;
  if(!offer || offer.id!==id)throw Error("Esta oferta ya no está pendiente.");
  if(!["accept","reject","delegate"].includes(action))throw Error("Respuesta de oferta no válida.");
