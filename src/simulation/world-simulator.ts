...
export function advanceWorldDayInPlace(next: GameState): GameState {
  ...
  if (next.runtime.day % 7 === 0 && hasActiveClubEmployment(next)) {
    ...
    // Optional pre-choice football context is a pure fixture projection. It is
    // materialized only after the authoritative match row exists and consumes no RNG draws.
