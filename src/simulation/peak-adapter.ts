...
export function adaptState26ToPeak(state:GameState,tags:State26Tag[]):void{
  if(state.professional.initializedAt26) return;
  const p=state.professional; const role=num(state.sport.roleScore,50); const market=num(state.reputation.marketHeat,30); const prestige=num(state.reputation.prestige,30);
  ...
  p.initializedAt26=true; state.flags.PEAK_26_ADAPTED=true;
}
