...
export function adaptState23ToAdult(state:GameState,tags:State23Tag[]):void {
  if(state.professional.initializedAt23) return;
  ...
  p.nationalRole="none"; p.nationalCaps=0;
  ...
  if(tags.includes("STATE23_ELITE_STARTER")) p.continentalCred=clamp(p.continentalCred+12);
  if(tags.includes("STATE23_MEDIA_PROFILE")) p.commercialPower=clamp(p.commercialPower+12);
  if(tags.includes("STATE23_INJURY_CROSSROADS")) p.bodyLoad=clamp(p.bodyLoad+12);
}
