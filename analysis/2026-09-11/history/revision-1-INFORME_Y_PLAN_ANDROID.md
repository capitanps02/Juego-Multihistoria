...
El trabajo visual puede ser online, pero el build Android requiere herramientas Android. Puede ejecutarse en un equipo configurado o en un servicio de integración continua online; PlayCanvas por sí solo no elimina ese paso. La elección y los secretos de firma se resuelven en T1 y T3. No se añaden servicios de servidor ni generación de texto por IA en tiempo de ejecución a esta v1: la biblioteca actual permite una experiencia local sin ese coste operativo.

## Objetivo final y límites de la primera versión

Se considerará alcanzado el objetivo cuando exista una app Android instalada desde Google Play en la región acordada, que permita crear un futbolista, completar su carrera, guardar y reanudar, recibir un epílogo comprensible y comenzar otra partida; con recursos y textos aprobados, pruebas físicas, documentación de publicación y seguimiento de las dos primeras semanas.
...
No se presupuestan partidos completos controlables, multijugador, mundo 3D explorable, doblaje integral, clubes licenciados, iOS, traducciones, comunidad, nube o tienda de compras dentro del juego. No son descartes de futuro: incorporarlos requiere horas, dependencias y criterios propios. Un juego de fútbol en tiempo real sería un proyecto adicional significativo, no una pequeña mejora de la presentación.
...
## Plan de pasadas con esfuerzo y porcentaje

T0 es esta auditoría y queda entregada. Los porcentajes de la tabla representan **el trabajo restante desde esta línea base hasta el lanzamiento estabilizado**. Empiezan en 0 %; no significan que el trabajo previo carezca de valor ni que hoy no exista motor. No se adopta el número de versión 0.8 como un 80 % de avance del videojuego.

Referencia inicial: una persona con capacidad efectiva de treinta horas semanales para ingeniería, contenido y QA, con asistencia de IA y decisiones del autor disponibles. Si el autor revisa pero no ejecuta trabajo técnico, esas horas de revisión no se convierten automáticamente en capacidad de desarrollo. La pregunta de disponibilidad sigue pendiente.

| Pasada | Entregable aceptable | Horas base | Rango de horas | Peso | Acumulado | Semana productiva de cierre orientativa |
|---|---|---:|---:|---:|---:|---:|
| T1 | Alcance, trazabilidad piloto y diseño técnico fijados | 50 | 40–60 | 6 % | 6 % | 2 |
| T2 | Sesión interactiva y guardado seguros | 100 | 80–120 | 11 % | 17 % | 5 |
| T3 | Demostración PlayCanvas instalada en Android | 100 | 80–120 | 11 % | 28 % | 9 |
| T4 | Ciclo de juego y primer tramo validados con personas | 100 | 80–120 | 11 % | 39 % | 12 |
| T5 | Contenido y memoria de la carrera reconciliados | 300 | 240–360 | 33 % | 72 % | 22 |
| T6 | Alfa completa y balance por rutas | 100 | 80–120 | 11 % | 83 % | 25 |
| T7 | Beta Android con rendimiento y estabilidad aceptados | 70 | 55–85 | 8 % | 91 % | 28 |
| T8 | Candidata publicada tras pruebas y revisión de tienda | 60 | 45–75 | 7 % | 98 % | 30 |
| T9 | Dos semanas de estabilización y entrega de operación | 20 | 15–25 | 2 % | 100 % | 30 de esfuerzo más observación |
| Total | Android v1 estabilizada | 900 | 715–1.085 | 100 % | — | 30 semanas de esfuerzo base |

Los pesos redondean la distribución de horas, no la importancia emocional de una función. Las semanas se calculan con horas acumuladas divididas por treinta y redondeadas hacia arriba; **no son fechas de entrega con contingencia**. Se permite solapamiento cuando hay recursos distintos, pero no se cuenta dos veces la capacidad de una misma persona.

La estimación de T5 cubre adaptación y revisión del guion ya escrito, no escribir de nuevo una novela de cien mil palabras. Su base reserva aproximadamente 230 horas para escenas y 70 para enlaces de memoria, NPC y epílogos; el primer lote determinará si ese ritmo es alcanzable. Si cada escena requiere cambios grandes de lógica, se revisará al alza.

### T1 y T2 para controlar la base

T1: fijar versión del guion, alcance de partidos, contenido de v1 y mapa de pantallas. Crear repositorio y lockfile si no existen en la copia de trabajo. Registrar los 87 IDs discrepantes y revisar un lote piloto de doce eventos de edades diversas. Definir correspondencias sin romper guardados. Acordar presupuesto de imágenes, orientación, cuenta de tienda y matriz de dispositivos.

Cierre T1: un documento de alcance aceptado y estimación actualizada con el esfuerzo real del lote. No exige terminar las 388 revisiones.

T2: introducir `GameSession`, evento pendiente persistido, comandos únicos y validación completa de guardados; separar consultas de mutaciones; impedir doble resolución; fijar instantáneas de edad; hacer que los gates fallen en el proceso de build; incorporar propuestas contractuales que no firmen solas.

Cierre T2: reanudación antes y después de elegir; cien ciclos de guardado y carga con decisiones pendientes sin divergencia; comandos repetidos no duplican efectos; corrupción detectada y copia anterior recuperada; migración de cada versión que se declare compatible. Las pruebas actuales continúan pasando o sus cambios están justificados con nueva referencia reproducible.

### T3 y T4 para demostrar que merece jugarse

T3: una escena de decisión con retratos, texto largo, resultado y guardado; una pantalla de estado; exportación web y APK instalable; funcionamiento offline; atrás, suspensión, recuperación y áreas seguras. Ensayar interfaz HTML/CSS y, donde haga falta, elementos nativos de PlayCanvas.

Cierre T3: mismo escenario y resultado en la ejecución automática, navegador y Android. Si el empaquetado, la legibilidad o el consumo no cumplen tras un ensayo acotado, elegir interfaz web más directa con el mismo motor. La reserva inicial de comparación es de doce a dieciocho horas dentro de T3; no abrir tres implementaciones completas.

T4: completar 18–20 con calidad representativa, primer tutorial breve, cinco o seis pantallas esenciales y un partido simulado que permita comprender la aportación del futbolista. Integrar consecuencias y una cadena de memoria. Sesiones observadas con ocho a doce personas, dos rondas de mejora incluidas.

Cierre T4: superar o explicar los criterios de comprensión y continuación descritos arriba; ninguna repetición genérica evidente en la demo; contrato jugable probado. Si el resultado no atrae, no multiplicar pantallas ni añadir edades para taparlo: reestimar el ciclo central.

### T5 y T6 para terminar la carrera

T5 se ejecuta en lotes de doce a veinte escenas por tramo, pasando cada lote por: fuente canónica, texto y decisiones, gates, outcomes contextuales, NPC, seeds, presentación y prueba. Primero quedan completos 20–23 y 23–26; después 26–30, 30–34 y 34+, manteniendo los callbacks hacia los tramos anteriores.

Cierre T5: matriz con 388 registros revisados, decisiones específicas, IDs trazables y desviaciones documentadas; cada una de las 210 semillas dispone de origen y lector o salida de epílogo verificable. No se exige que todas se activen en una misma partida. Los veinte NPC tienen presencia y evolución definidas según relevancia; no se fuerza a todos a aparecer en cada carrera. Los finales muestran texto y hechos de la partida, no códigos internos.

T6: completar varias carreras manuales y simulaciones estratificadas. Crear políticas semánticas —prudencia, riesgo, dinero, minutos, lealtad, movilidad— que seleccionen intención, no posición A/B/C/D. Cubrir los quince perfiles de prueba de la sección 26.26 del guion. Medir oportunidades, lesiones, contratos, callbacks, densidad, envejecimiento y riqueza por separado.

Cierre T6: ejecutar hasta diez mil carreras en la candidata de balance, según coste medido; cero estados imposibles o bloqueos reproducibles; cobertura por evento, elección y outcome con excepciones documentadas para rarezas. Los casos extraordinarios se prueban también con estados dirigidos, sin introducir esos estados artificiales en partidas normales. Reportar densidad por ruta; investigar rutas por debajo del 70 % de la media. Auditar veinte epílogos concretos y el comportamiento de carreras largas.

### T7 a T9 para publicar y mantener

T7: QA sobre al menos tres teléfonos físicos de distinta capacidad, con versiones de Android y WebView documentadas; añadir emuladores para tamaños y versiones. Sesiones largas, almacenamiento escaso, recursos faltantes, modo avión, regreso del fondo, actualización desde la beta anterior, texto ampliado y navegación accesible.

Cierre T7: cero incidencias críticas o altas abiertas, ningún guardado perdido en la matriz de interrupción, límites de rendimiento cumplidos en los dispositivos comprometidos y al menos cinco carreras completas realizadas por personas entre las rutas principales. Las incidencias menores restantes se registran con decisión de publicación.

T8: ficha, capturas reales, clasificación de contenido, privacidad y declaración de datos consistentes con las funciones incluidas; identidad y firma configuradas; AAB verificado; pruebas internas y cerradas; respuesta a revisión. Preparar una versión correctiva: una aplicación instalada no puede desactualizarse simplemente bajando `versionCode`.

Cierre T8: disponible desde Google Play para el público acordado. Enviar la solicitud o conseguir un APK no cierra esta pasada. Se controla por separado el trabajo terminado y la espera de aprobación de terceros.

T9: observar dos semanas, resolver fallos relevantes, comprobar guardados tras actualizar y entregar instrucciones de build, publicación y recuperación. Cierre: sin incidencias críticas o altas abiertas y versión identificable instalada correctamente. No incluye desarrollo continuo de nuevas funciones.

## Calendario realista y contingencia

| Capacidad efectiva para todo el trabajo | Esfuerzo base | Reserva del 20 % | Espera y observación adicionales estimadas | Calendario base orientativo |
|---|---:|---:|---:|---:|
| 30 h por semana | 30 semanas | 6 semanas | 2–4 semanas | 38–40 semanas |
| 20 h por semana | 45 semanas | 9 semanas | 2–4 semanas | 56–58 semanas |
| 10 h por semana | 90 semanas | 18 semanas | 2–4 semanas | 110–112 semanas |

El cálculo supone continuidad de trabajo, acceso a probadores y herramientas, y que parte de la prueba cerrada puede solaparse con la preparación de tienda. Vacaciones, falta de materiales, incumplimiento de criterios de diversión o rechazos prolongados añaden tiempo. Los 2–4 semanas son una provisión, no un plazo prometido por Google.

Con treinta horas semanales, el rango de esfuerzo de 715–1.085 h más un 20 % de reserva y 2–4 semanas adicionales produce aproximadamente **31–48 semanas**. La línea central es 38–40. Una demo Android llega antes: T1–T3 suman 250 horas base, unas nueve semanas productivas; planificar diez a doce semanas naturales con margen. Si ya se dispone de UI utilizable y herramientas configuradas, ese hito se reestima con evidencia.

Si el proyecto arrancara el lunes 14 de septiembre de 2026 con esa capacidad, la línea central apuntaría a junio de 2027. Es un ejemplo de calendario condicionado, no una fecha contratada; las vacaciones tendrían que incorporarse expresamente. Un equipo pequeño puede reducir calendario repartiendo contenido, interfaz y pruebas, pero no dividir automáticamente por su número de integrantes ni eliminar dependencias.

La ayuda de IA puede acelerar edición, scripts y pruebas. No se le asigna una velocidad garantizada de producción, ni se cuentan como trabajo completado las horas en que una herramienta esté disponible. Las horas de esta estimación son esfuerzo equivalente de producción revisada. Tras T1 y T4 se sustituirán por el ritmo observado.

## Seguimiento y reglas para adherirse al plan

El avance ganado se calcula con `suma de peso de entregables aceptados`. Cada pasada se divide en tareas de cuatro a dieciséis horas con evidencia concreta. Una tarea solo gana su peso al cumplir su criterio de aceptación; trabajar en ella o generar archivos no basta. Se registran por separado horas consumidas, porcentaje ganado, defectos y trabajo pendiente.

La auditoría T0 está entregada. T1–T9 tienen 0 % ejecutado en este plan. Para el producto previo sería engañoso inventar un porcentaje exacto: existe un núcleo de simulación operativo, contenido mayoritariamente provisional y todavía no se ha evaluado la interfaz. El porcentaje defendible empezará al fijar la matriz de entregables.

Cadencia propuesta: sesiones de trabajo con evidencia al cierre; una revisión semanal de avance y riesgos; demostración utilizable cada dos semanas; una decisión formal al cerrar cada T. En cada parte se informa de lo terminado, lo bloqueado, el esfuerzo restante y la siguiente prueba. Esta cadencia es el modo de trabajo propuesto, no una automatización creada ni una promesa de ejecución desatendida.

Si una pasada consume el 80 % de sus horas con menos del 60 % de sus entregables aceptados, se revisa el alcance y la previsión antes de seguir gastando. Si la desviación prevista supera el 15 % o dos semanas, se registra una nueva previsión. No se cambia silenciosamente la fecha original: se mantienen línea base, previsión actual y razón de cambio.

Todo añadido —una pantalla, tipo de partido, monetización o paquete de arte— debe indicar objetivo, horas, dependencia, criterio y efecto sobre fecha. La reserva cubre incertidumbre del trabajo previsto; no financia todas las funciones nuevas. Ante presión de plazo, recortar variedad audiovisual o extras antes que guardado, claridad de decisiones o pruebas de integridad.

Responsabilidades: la implementación, trazabilidad técnica y QA automatizado corresponden a ingeniería; intención narrativa, alcance y aceptación visual al autor; usabilidad y diversión a participantes externos; publicación e identidad a la persona titular de las cuentas. Una misma persona puede asumir varios papeles, pero hay que reservar tiempo para todos.
...
Una previsión monetaria real requiere multiplicar las horas acordadas por el coste de quienes ejecuten el trabajo y sumar licencias, recursos y servicios. No es correcto llamar gratuito al desarrollo porque lo haga el autor. Tampoco hay datos para prever rentabilidad o justificar adquisición pagada de usuarios. La monetización queda por decidir; introducir publicidad, compras integradas o cuentas añadiría implementación, pruebas y declaraciones de datos que no están dentro de las 900 horas base.
...
## Evidencia reproducible y siguiente entrega

Los resultados medidos se conservan en `runtime-audit.json`, `content-audit.json`, `canonical-id-audit.json` y `source-manifest.json`. `audit.mjs` contiene las pruebas independientes. `plan-seguimiento.json` conserva pesos, horas, dependencias y criterios de aceptación para actualizar el avance sin cambiar la línea base. La copia compilada permite repetirlas; TypeScript 5.8.3 se descargó en la carpeta de auditoría con autorización del usuario.
...
La siguiente entrega recomendada es T1: especificación de la primera versión, tabla de correspondencias piloto y doce escenas auditadas con esfuerzo real registrado. Ese trabajo permite convertir este calendario condicionado en una línea base de ejecución con responsables y capacidad confirmados.
