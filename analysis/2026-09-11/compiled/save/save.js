...
export function serializeSave(state, pretty = false) { return JSON.stringify(state, null, pretty ? 2 : 0); }
...
export function loadSave(raw) {
    ...
    if (state.schemaVersion !== CURRENT_SCHEMA_VERSION)
        throw new Error(`Save schema ${String(version)} no compatible con motor v0.8 (schema ${CURRENT_SCHEMA_VERSION}).`);
    if (!state.rngState?.narrative || !state.rngState?.football || !state.rngState?.microfeed || !state.rngState?.qa)
        throw new Error("Save corrupto: faltan streams RNG separados.");
    if (!state.runtime || !state.professional)
        throw new Error("Save corrupto: falta runtime/professional.");
    return state;
}
